...
# --- 5. 主处理函数 ---
def process_image_with_mmpose(image_path, conf_threshold, output_path):
    """处理单张图片并保存结果"""
    ...
    print(f"  検出された遊具: {len(detected_equipments)}個")
    ...
    print(f"  検出された人: {len(person_boxes_np)}人")
    ...
    print(f"  推定された姿勢: {len(all_poses_with_boxes)}個")
    ...
    for eq in detected_equipments:
        ...
        print(f"  [{eq['name']}]の上: {num_people}人")
...
def process_folder(folder_path, conf_threshold):
    """处理文件夹中的所有图片"""
    ...
    if not image_files:
        print(f"警告: フォルダ内に画像ファイルが見つかりません '{folder_path}'")
        return
    
    print(f"\n合計 {len(image_files)} 枚の画像を処理します...\n")
    print("=" * 60)
    
    success_count = 0
    for idx, image_file in enumerate(image_files, 1):
        print(f"\n[{idx}/{len(image_files)}] 処理中: {image_file.name}")
        
        # 生成输出文件名：原文件名_result.扩展名
        output_filename = f"{image_file.stem}_result{image_file.suffix}"
        output_path = folder / output_filename
        
        # 处理图片
        if process_image_with_mmpose(str(image_file), conf_threshold, str(output_path)):
            success_count += 1
    
    print("=" * 60)
    print(f"\n処理完了: {success_count}/{len(image_files)} 枚の画像を正常に処理しました")


# --- 脚本入口 ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='フォルダ内の全画像をYOLOv8とMMPose(HRNet)で処理し、危険な遊び方をスコアリングします。')
    parser.add_argument('folder_path', type=str, help='処理対象の画像フォルダのパス')
    parser.add_argument('--conf', type=float, default=0.25, help='YOLOv8遊具検出の信頼度の閾値(デフォルト: 0.25)')
    args = parser.parse_args()
    
    process_folder(args.folder_path, args.conf)